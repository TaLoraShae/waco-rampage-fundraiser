...
import { togglePlayerActive, deletePlayerAction, resetPlayerTotals } from "@/app/admin/data-actions";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Link href="/admin/players/new" className="inline-flex items-center rounded-full bg-rampage-purple text-white text-sm font-semibold px-4 py-2 hover:bg-rampage-purple-dark transition focus-ring">
          + Add Player
        </Link>
      </div>

      {searchParams.success && MESSAGES[searchParams.success] && (
        <p className="text-sm rounded-lg bg-green-50 border border-green-200 text-green-700 p-3">{MESSAGES[searchParams.success]}</p>
      )}
      {searchParams.error && (
        <p className="text-sm rounded-lg bg-red-50 border border-red-200 text-red-700 p-3">
          {searchParams.error === "duplicate-slug" ? "That slug is already in use. Please choose a unique slug." : searchParams.error === "missing" ? "Name and slug are required." : searchParams.error}
        </p>
      )}
      ...
      <div className="bg-white rounded-2xl border border-black/5 shadow-card-light overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {players.map((p) => {
              const raised = data.getPlayerRaisedCents(donations, p.id);
              return (
                <tr key={p.id} className="border-t border-black/5">
                  <td className="px-4 py-3 text-rampage-gray">{p.display_order}</td>
                  <td className="px-4 py-3 font-medium text-rampage-charcoal">
                    {p.display_name}
                    {p.is_general_fund && (
                      <span className="ml-2 text-[10px] uppercase tracking-wide bg-rampage-purple/10 text-rampage-purple-dark px-2 py-0.5 rounded-full">General Fund</span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3">{formatCents(p.goal_cents)}</td>
      ...
      <p className="text-xs text-rampage-gray">
        Players with donation history can&apos;t be deleted — deactivate them instead. &ldquo;Reset Totals&rdquo; is
        restricted to Owner/Treasurer since it deletes donation records.
      </p>
    </div>
  );
}
